...
def GiveMeMyHistos(var):
    ...
    p = r.PlotToPyC(r.TString(vl.varList[var]['var']), r.TString(UsedCut), r.TString('ElMu'), int(len(vl.varList[var]['recobinning']) - 1), binning, r.TString(var), r.TString(vl.varList[var]['xaxis']))
    ...
    pdf     = r.PDFToPyC(r.TString(pathToTree), r.TString("TTbar_Powheg"), r.TString(NameOfTree), r.TString(UsedCut), r.TString("ElMu"), r.TString(vl.varList[var]['var']), len(vl.varList[var]['recobinning']) - 1, binning, r.TString(''));
    pdf.verbose = False
    pdf.verbose = True
...
def GiveMeMyTestHistos(var):
    binning = array('f', vl.varList[var]['recobinning']) # For some reason, PyROOT requires that you create FIRST this object, then put it inside the PlotToPyC.
    p = r.PlotToPyC(r.TString(vl.varList[var]['var']), r.TString(UsedCut), r.TString('All'), int(len(vl.varList[var]['recobinning']) - 1), binning, r.TString(var), r.TString(vl.varList[var]['xaxis']))
    p.SetPath(pathToTree); p.SetTreeName(NameOfTree);
    p.SetLimitFolder('./temp/{var}_/'.format(var = var))
    p.SetPlotFolder('./temp/{var}_/'.format(var = var))
    p.SetPathSignal(pathToTree);
    ...
    p.AddSample('TTTo2L2Nu',             'ttbar',    r.itBkg, 633, systlist, opts)
    p.AddSample('b_bbar_4l',             'ttbartw',  r.itBkg, 633, systlist, opts)
    ...
    p.AddSample("tW_noFullHad",           "tW",       r.itBkg, r.TColor.GetColor("#ffcc33"), systlist, opts)
    p.AddSample("tbarW_noFullHad",        "tW",       r.itBkg, r.TColor.GetColor("#ffcc33"), systlist, opts)
    ...
    p.PrintSystYields()
    p.NoShowVarName = True;
    p.SetOutputName("Histos_{var}".format(var = var));
    p.SaveHistograms();
    del p
    return
...
def lazyoptimisation(tsk):
    ...
    elif ty == "test":    return GiveMeMyTestHistos(var)
