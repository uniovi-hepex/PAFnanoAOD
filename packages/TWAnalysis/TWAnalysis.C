...
//   }
  ...
  if ((DressNLeps >= 2) && (DressNJets == 1) && (DressNBJets == 1) && ((DressLeptons.at(0).p + DressLeptons.at(1).p).M() > 20) &&
      (DressLeptons.at(0).Pt() > 25) && (DressLeptons.at(1).Pt() > 20) &&
      (GenChannel == iElMu) && !TDressIsSS && (DressNLooseCentral == 1)) {
  ...
  if ((TNSelLeps >= 2) && passTrigger && passMETfilters && ((selLeptons.at(0).p + selLeptons.at(1).p).M() > 20)
    && (selLeptons.at(0).Pt() > 25) && (selLeptons.at(1).Pt() > 20)
    && (TChannel == iElMu) && (!TIsSS)) {
...
Float_t TWAnalysis::getMiniMax(Float_t ml1j1, Float_t ml1j2, Float_t ml2j1, Float_t ml2j2) {
